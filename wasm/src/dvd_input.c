...
struct dvd_input_s {
  ...
  /* webdvd: on-demand VOB fetching over the network instead of MEMFS.
   * For *.VOB devices we never read from `fd`; blocks are fetched lazily
   * from the server via the JS bridge below. `cur_block` tracks the read
   * position (advanced by file_seek/file_read, since we bypass the fd). */
  char *path;       /* strdup of the open target, NULL if not tracked */
  int   is_vob;     /* 1 if path ends in .VOB → fetch on demand */
  int   cur_block;  /* next 2048-byte block to read */
};

/* webdvd: async JS bridge (defined via EM_ASYNC_JS in glue.c). Fetches
 * `count` blocks starting at `start_block` of the VOB named by `path` into
 * `dst`; returns blocks read, or -1 on error. Suspends the WASM stack
 * (Asyncify) across the fetch. */
extern int dvdread_fetch_blocks(const char *path, int start_block,
                                int count, void *dst);

static int path_is_vob(const char *p)
{
                             ...
                             dvd_reader_stream_cb *stream_cb UNUSED)
{
  ...
  /* webdvd: remember the path so VOB reads can be served on demand. */
  dev->path = strdup(target);
  dev->is_vob = path_is_vob(target);
  dev->cur_block = 0;
...
{
  /* webdvd: VOBs are fetched on demand — track position, skip the MEMFS fd
   * (which is a 0-byte placeholder). */
  if(dev->is_vob) {
    dev->cur_block = blocks;
    return blocks;
  }
  off_t pos;
		     ...
		     int flags UNUSED)
{
  ...
  /* webdvd: fetch VOB blocks on demand from the server (Asyncify suspends
   * the WASM stack across the network round-trip). The MEMFS fd is a 0-byte
   * placeholder and is never read. */
  if(dev->is_vob) {
    int got = dvdread_fetch_blocks(dev->path, dev->cur_block, blocks, buffer);
    if(got < 0)
      return got;
    dev->cur_block += got;
    return got;
